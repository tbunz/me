...
::text-block
[Kalshi](https://kalshi.com/), a popular prediction market platform, runs a [liquidity incentive program](https://help.kalshi.com/en/articles/13823851-liquidity-incentive-program) that pays traders to rest orders on the book (even if these orders are never filled). For a given market, they establish a reward pool with a total cash value and a time period. Over this time period, Kalshi looks at the book and scores each trader, tracking how large orders are and how far they are from the best price. At the end of the time period, traders are ranked by score, and relative shares of the reward pool are paid out. These are known as "liquidity rewards". They are a subsidy that incentivizes traders to place resting orders at (or near) the best price. Generally speaking, traders who place these orders can be classified as "market makers". 
::
...
::text-block
The system is a Python project with four layers. An "operator" interacts with the system to run analysis and initiate trading for a market. As detailed below, the operator is Claude (but the system allows any agent, LLM or human, to act). The architecture:
::
...
<br>
Over the course of 16 weeks I have been able to turn a profit, maintaining a steady (albeit modest) income with my strategy while using a relatively small amount of personal capital. The gains are unimpressive in absolute terms, but the costs of the project are low: 
